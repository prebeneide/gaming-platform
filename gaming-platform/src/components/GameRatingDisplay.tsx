...
export default function GameRatingDisplay({ ratings, className = "" }: GameRatingDisplayProps) {
  ...
  if (!ratings || ratings.length === 0) {
    return (
      <div className={`bg-neutral-900 rounded-xl p-6 border border-neutral-800 ${className}`}>
        <div className="text-center text-neutral-400">
          <div className="text-4xl mb-2">🎮</div>
          <div className="text-lg font-medium">Ingen rating ennå</div>
          <div className="text-sm">Spill noen matcher for å få rating!</div>
        </div>
      </div>
    );
  }

  const selectedRating = ratings.find(r => r.gameName === selectedGame) || bestRating;
    ...
    <div className={`bg-gradient-to-br from-neutral-900 via-neutral-900 to-neutral-800 rounded-xl p-6 border border-neutral-700 shadow-xl ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-neutral-400 text-sm">Din beste rating: {bestRating?.gameName}</p>
        </div>
            ...
            {sortedRatings.map((rating) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${getRatingColor(rating.rating)}`} />
